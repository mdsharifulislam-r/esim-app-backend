...
if (!moduleName) {
  console.log('Please provide a module name, e.g., node generateModule UserProfile');
} else {
...
function updateRouterFile(folderName: string, camelCaseName: string): void {
  ...
  const apiRoutesRegex = /const\s+apiRoutes(?:\s*:\s*[^=]+)?\s*=\s*\[(?<items>[\s\S]*?)\]\s*;?/m;
  const match = apiRoutesRegex.exec(routerFileContent);
 
  if (match) {
    const currentRoutes = match[1].trim();
    if (!currentRoutes.includes(routeEntry)) {
      const updatedRoutes = currentRoutes ? `${currentRoutes}\n  ${routeEntry}` : `${routeEntry}`;
