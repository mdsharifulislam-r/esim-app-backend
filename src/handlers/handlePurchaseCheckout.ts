        ...
        [
          {
            ...
            coupon: getCopoun?._id,
            coupon_str: payload.coupon,
          },
        ],
        { session: mongoSession },
      );
      const discount = await HoldDiscount.findOne({
        refferal_code: payload.coupon,
        status: 'active',
      });
      if (discount) {
        await HoldDiscount.findOneAndUpdate(
          {
            refferal_code: payload.coupon,
            status: 'active',
            owner: payload.description,
          },
          { status: 'used' },
        ...
        if (payload.commission) {
          ...
          if (
            user?.stripeAccountInfo?.loginUrl &&
            user?.stripeAccountInfo?.accountId
          ) {
